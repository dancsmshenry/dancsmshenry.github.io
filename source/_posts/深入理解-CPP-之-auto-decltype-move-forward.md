...
其实左值，右值，本身是没有任何意义的，必须要结合具体的场景来分析。

因为在一些推导过程中，一些变量会被推导为其他的类型，这个时候再去讨论原来是左值，还是右值，是没有意义的了
...
std::vector<int> vec;
vec.push_back(1);
//	注：push_back 是右值引用接的，因此这里会被推导为一个右值
```
...
2025.09.23 错了，push_back 的实现上是万能引用

一些理解：

- Int & a = 10; 会编译抱错，猜测编译器确实给 10 找了块地方放，但是可能这块地方不能被修改，所以编译报错
- Const int & a = 10; 不会编译报错，是因为这块地方也不能修改
